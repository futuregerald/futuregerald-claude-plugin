...
2. **Downgrade?** Can you state the shape of a correct answer before dispatching ("come back with `file:line` and the caller list")? → **reduce the reasoning budget** — a smaller model, or the same model at a lower thinking level where your provider offers one. A small fast model for known-name lookups, a mid-tier model for multi-step exploration. If you cannot state the shape, that is judgment — keep it at the orchestrator's model. **Never trade output quality for a cheaper model.**
