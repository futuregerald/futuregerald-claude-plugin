...
1. **Can a pipe or ranged read get it?** `| tail`, `grep -c`, `sed -n 'A,Bp'`, a ranged read. ~200 tokens. Capture the exit code *before* piping — `cmd | tail` exits 0 even when `cmd` failed. If yes, do this and stop.
