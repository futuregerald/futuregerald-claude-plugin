...
data structure the HTML page renders from, so the two cannot drift.
...
Per `references/forecast/capacity-model.md`. Show the arithmetic, not just the result: productive weeks,
...
## The HTML page: `assets/forecast.html`

The page shares the pulse template's colours, type and components, so a forecast and a pulse read as
one family. Fill its placeholders from the same data structure as the Markdown report.
...
`.stack`, so a reader can compare them by eye. `left`/`width` go in inline `style` on `.band`,
`.demand`, `.opt`, `.pess`, `.seg` and `.frame`.

**`.run` gets a class from the running total:** `fits` when the pessimistic running total is inside
the band's lower edge, `tight` when only the optimistic one is, `over` when neither is.

Every ticket key and PR number is a link, as in a pulse.
