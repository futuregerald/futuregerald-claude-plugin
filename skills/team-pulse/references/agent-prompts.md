...
Cloud ID: 0d3ebe47-c776-4f60-9590-070d19b283f9
...
Repos: cobalthq/cobalt-pentest-api, cobalthq/cobalt-app-web, cobalthq/cobalt-admin-web
...
For each repo, run:
gh pr list --repo cobalthq/{REPO} --state all {AUTHOR_FLAG} --limit 20 \
...
```
Search GitHub for PRs reviewed by {HANDLE} across Cobalt repos since {START_DATE}.

gh search prs --reviewed-by {HANDLE} --owner cobalthq --updated ">={START_DATE}" \
