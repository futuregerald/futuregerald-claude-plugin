...
3. Association names DIFFER between repos (e.g., pentest-api uses `:csm`, admin-api uses `:cs_assignee` — both resolve to `cs_assignee_id`)
...
**Anti-pattern: raw FK column names.** Don't recommend `where(orgs: {cs_assignee_id: assignee&.id})` when the Rails-idiomatic fix is `where(orgs: {csm: assignee})` using the correct association name.

> **Case study — DL-2133:** Grooming claimed `orgs: {cs_assignee: assignee}` was referencing a "removed column" and flagged 8 files as buggy. In reality, `cs_assignee` wasn't an association name on the pentest-api Org model (which uses `belongs_to :csm`). Most flagged files were correct in their own repo. The fix was a one-file association name correction, not an 8-file rewrite. Root failure: never read the Org model's belongs_to definitions.
...
`params.require(:pentest).permit(:title, :description)` is the controller-level protection. Internal service objects that receive pre-validated hashes do NOT need strong params.
...
`resources :pentests` generates 7 routes. `member` routes include `:id`, `collection` routes don't.
