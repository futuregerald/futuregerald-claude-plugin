...
Close-but-wrong names destroy trust. _Added after a false-positive on cobalt-pentest-api#7557 where `DestroyResource` was cited but the actual class was `DestroyWithInvoiceUpdate`._
...
_Added after DL-1871 where three iterations investigated the wrong code path because no one challenged the initial hypothesis._
...
_Added after DL-1871 where investigation focused on the display filter (serializer) instead of the write path (proficiency pipeline), wasting multiple iterations._
...
_Added after DL-2133 where grooming flagged 8 files as buggy from grep results without reading the Org model's belongs_to definitions. Most files were correct in their own repo. A "removed column" root cause was fabricated — the actual issue was a wrong association name._
...
2. **What you found** — "The model defines `belongs_to :csm, foreign_key: :cs_assignee_id`"
3. **How it connects** — "Therefore `where(orgs: {csm: assignee})` resolves to `cs_assignee_id`, which is correct"
...
_Added after DL-2133 where "HIGH confidence (verified)" was claimed without having read the model file that would have disproved the hypothesis._
