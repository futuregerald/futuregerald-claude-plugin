...
- For backend tickets: ALWAYS search both `cobalt-pentest-api` AND `cobalt-admin-api`. Budget applies per repo.
- `cobalt-admin-api` is mid-migration into `cobalt-pentest-api` (under `components/admin/`). Code may exist in both.
