# Graphify Pipeline Steps (Reference)

Detailed step-by-step pipeline for building a knowledge graph. See [../SKILL.md](../SKILL.md) for usage and overview.
...
**IMPORTANT - model:** Always use `model: "sonnet"` for extraction subagents. Sonnet is the required model for all semantic extraction work — it provides the best balance of extraction quality vs token cost. Do NOT use Opus (too expensive for structured extraction) or Haiku (insufficient quality). This is not optional.
...
See [subcommands.md](subcommands.md) for Steps 6 (visualization/Obsidian/HTML), 6b (Wiki), 7 (Neo4j/SVG/GraphML/MCP exports), 8 (token benchmark), and 9 (cleanup and report).
