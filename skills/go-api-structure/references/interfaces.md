...
`decode`, `encode` and `decodeStatus` are package-level helpers declared alongside this file in
[`example/internal/httpapi/encoding.go`](../example/internal/httpapi/encoding.go) — the file is
complete, but `package httpapi` is not one file.
